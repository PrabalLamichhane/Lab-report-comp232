...
CREATE USER 'user_1'@'%' IDENTIFIED BY 'root';
GRANT CREATE, UPDATE, SELECT, DELETE ON CustomerInsuranceProfiles_DB TO 'user_1'@'%';

CREATE USER 'user_2'@'%' IDENTIFIED BY 'root';
GRANT CREATE, UPDATE, SELECT, DELETE on CustomerInsuranceProfiles_DB TO 'user_2'@'%';
...
UPDATE insurancepolicy SET premium = premium - 50 WHERE policyID = 1;
...
LOCK TABLES insurancepolicy READ;

SELECT * FROM insurancepolicy WHERE policyID = 1;
...
SELECT * FROM insurancepolicy WHERE policyID = 1 FOR UPDATE;
...
SELECT * FROM insurancepolicy WHERE policyID = 1 LOCK IN SHARE MODE;
